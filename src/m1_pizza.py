...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(150.0, 300.0),  # All numbers are approximate.
                rg.Point(100, 350),
                rg.Point(50.5, 300),
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # : 3. Implement this TEST function.
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for i in range(0, number_of_points):
        c = rg.Circle(generate_points_on_circle(circle, number_of_points)[i], 10)
        c.fill_color = color
        c.attach_to(window)
        generate_points_on_circle(circle, number_of_points)[i].attach_to(window)
    window.render()

def run_test_pizza():
    """ Tests the   pizza   function. """
    # -------------------------------------------------------------------------
    # D: 5. Implement this TEST function.
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  20 slices, thin (thickness=1) yellow lines.'
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for i in range(number_of_slices):
        line = rg.Line(generate_points_on_circle(circle, number_of_slices)[i], circle.center)
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    # D: 7. Implement this TEST function.
    ...
    # -------------------------------------------------------------------------
    title = 'POLYGON test 3: 5 with thin brown lines'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'brown', 1)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        if k > 0:
            ...
            line = rg.Line(points[k], points[number_of_segments - 1])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    # D: 9. Implement this TEST function.
    ...
    # Test 3 (on another window):
    title = ('FANCY POLYGON test 4:  50 brown lines on a khaki circle,'
             + ' hops = 25.')
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.fill_color = 'khaki'
    fancy_polygon(window, circle, 50, 25, 'brown', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for i in range(number_of_lines):
        if i + hops_to_next_point < number_of_lines:
            line = rg.Line(points[i], points[i + hops_to_next_point])
        else:
            line = rg.Line(points[i], points[i + hops_to_next_point - number_of_lines])
        line.thickness = thickness
        line.color = color
        line.arrow = 'last'
        line.attach_to(window)
